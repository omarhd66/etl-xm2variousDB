...
my $sdn= 'C:\Users\Omar\Desktop\Stage Nokia\Perl XML\XML2SMSACCESS\BSC-400301.accdb';

my $dbh = DBI->connect("dbi:ODBC:driver=Microsoft Access Driver (*.mdb, *.accdb);dbq=$sdn");
...
foreach my $class (@allcasses) {
		...
		
	{
		...
		#############################################
		#Ajouter les autres attributs en tête dans DB
		################CSV##########################
		$len = @ExhausTable;
		foreach (@ExhausTable){
